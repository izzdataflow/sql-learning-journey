...
SELECT 
	OrderID,
    CreationTime,
	CAST(DATETRUNC(month, CreationTIme) AS DATE) AS SOMONTH,
	EOMONTH(CreationTime) AS EOMONTH,
	DATETRUNC(year, CreationTime) AS year_dt,
	DATETRUNC(day, CreationTime) AS day_dt,
	DATETRUNC(minute, CreationTime) AS minute_dt,
	'2026-3-17' AS HardCoded,
	GETDATE() AS Today,
...
SELECT 
	...
	DATEADD(day, -10, OrderDate) TenDaysAgo
FROM Sales.Orders;
...
SELECT
	...
	RANK() OVER(ORDER BY COALESCE(AVG(DayUntilNextOrder), 99)) RankAvg
FROM(
SELECT
	OrderID,
	CustomerID,
...
SELECT
	...
	Price,
	--SubQuery
	(SELECT COUNT(1) FROM Sales.Orders) TotalOrders
FROM Sales.Products;
...
ORDER BY COALESCE(CustomerRanK, 99)
...
SELECT 
	...
	idx.name IndexName,
	idx.type_desc IndexType,
	COUNT(1) OVER (PARTITION BY tbl.name, col.name) ColumnCount
FROM sys.indexes idx
...
SELECT
	...
	sp.rows 'Rows',
	sp.modification_counter ModificationsSinceLastUpdate
FROM sys.stats s
JOIN sys.tables t
...
--33.SQL Execution Plan and Hints
SELECT *
...
SELECT 
	...
	c.Country
FROM Sales.Orders o
LEFT JOIN Sales.Customers c WITH (INDEX([])) --WITH (FORCESEEK)
ON o.CustomerID = c.CustomerID
--OPTION (HASH JOIN)
...
CREATE CLUSTERED COLUMN INDEX Idx_Orders_Columnstore ON Sales.Orders
...
CREATE TABLE CustomersInfo(
	...
	FirstName VARCHAR(MAX) NOT NULL, --VARCHAR(50)
	LastName VARCHAR(50) NOT NULL,
	Country VARCHAR(255) NOT NULL, --VARCHAR(50)
